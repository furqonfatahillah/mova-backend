...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        Schema::table('businesses', function (Blueprint $table) {
            if (!Schema::hasColumn('businesses', 'coin_balance')) {
                $table->decimal('coin_balance', 14, 2)->default(0.00)->after('status');
            }
            if (!Schema::hasColumn('businesses', 'coins_per_transaction')) {
                $table->decimal('coins_per_transaction', 8, 2)->default(1.00);
            }
            if (!Schema::hasColumn('businesses', 'low_coin_threshold')) {
                $table->integer('low_coin_threshold')->default(2000);
            }
        });
